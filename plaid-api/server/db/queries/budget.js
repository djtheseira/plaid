...
const getBudgetActualSumComparisons = async (userId, showAnnual) => {
    const query = {
        text: `
            ...
            FROM (
                ...
                FROM transactions t
                JOIN plaid_categories pc ON t.category = pc.category_type || '_' || pc.category
                ...
                AND t.user_id = $1
                AND transaction_date > (current_date - interval '1 ${showAnnual ? "year" : "month"}')
            	GROUP BY pc.category_id
            ) x 
            RIGHT JOIN plaid_categories pc ON pc.category_id = x.category_id
            LEFT JOIN budgets b ON b.category_id = pc.category_id
